...
OPT_LONG=('allsource' 'check' 'clean' 'cleanbuild' 'config:' 'force' 'geninteg'
		  ...
		  'distrovars' 'skip-makedeb-warning')
...
while true; do
	case "$1" in
		...
		--skip-makedeb-warning)  SKIP_MAKEDEB_WARNING=1 ;;
		--)                      shift; break ;;
	esac
	shift
done
...
if ! (( ${SKIP_MAKEDEB_WARNING} )); then
	...
	error "If you'd like to continue, use the '--skip-makedeb-warning' option."
	exit 1
fi
